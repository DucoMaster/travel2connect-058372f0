...
import { useGuideById } from "@/actions/guide-quries";
import { usePackagesByGuideId } from "@/actions/package-queries";
import { Spinner } from "@/components/spinner";

const GuideDetail = () => {
  const { id } = useParams();
  const [activeTab, setActiveTab] = useState("about");
  const { data: guide, isLoading: guideLoading, isError } = useGuideById(id);
  const { data: guidePackages, isLoading: packagesLoading } =
    usePackagesByGuideId(id);
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-end -mt-12">
              <Avatar className="h-24 w-24 border-4 border-white shadow-md">
                <AvatarImage
                  ...
                  alt={guide.name || guide.email}
                />
                <AvatarFallback className="bg-travel-200 text-travel-700 text-2xl">
                  {(
                    guide.name?.charAt(0) || guide.email.charAt(0)
                  ).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              ...
              >
                ...
                {guidePackages.length > 0 ? (
                        ...
                        <div className="flex flex-col sm:flex-row">
                          ...
                          <CardContent className="flex-1 p-4">
                            <div className="flex justify-between mb-2">
                              <Badge
                                variant="outline"
                                className="bg-travel-50 text-travel-700"
                              >
                                {pkg.category}
                              </Badge>
                  ...
                  <p className="text-gray-600">
                    This guide doesn't have any packages available yet.
                  </p>
                )}
              </TabsContent>
